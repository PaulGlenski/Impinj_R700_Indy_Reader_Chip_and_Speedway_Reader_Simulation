...
class PhaseNoise(object):
    ...
    def __init__(self, freq=None, phase_noise=None, fs=1, delta_f=None):
        ...
        self._gen_new_sample_set()

    def _interpolate_to_linear(self, freq, phase_noise, delta_freq=None):
        """
        Interpolate
        """
        ...
        lin_phase_noise = np.empty(int(self.n_fft / 2))
        spec_idx = 0
        cur_slope = 0.0
        idx = 0
        ...
        return {'freq': lin_freq, 'phase_noise': lin_phase_noise}

    def _gen_new_sample_set(self):
        #sigma = math.sqrt(float(self.Fs / self.n_fft))
        #sigma = math.sqrt(float(self.Fs / self.n_fft))
        sigma = float(self.n_fft/2)
        white_noise_freq_samples = int(np.random.normal(0, sigma, self.n_fft/2) + 1j * np.random.normal(0, sigma, self.n_fft/2))
    ...
    def gen_noise(self, num_samples):
        ...
        for t in range(num_times-1):
            self._gen_new_sample_set()
            np.append(noise, self.noise_samples)

        self._gen_new_sample_set()
        np.append(noise, self.noise_samples[0:last_samples])
